...
class Spar(Component):
    # design variables 
    wall_thickness = Array([0.05, 0.02, 0.02, 0.02],iotype='in', units='m',desc = 'wall thickness of each section')
    number_of_rings = Array([1, 4, 4, 13],iotype='in',desc = 'number of stiffeners in each section')
    neutral_axis = Float(0.272,iotype='in',units='m',desc = 'neutral axis location')
    # inputs 
    ...
    water_depth = Float(130.,iotype='in', units='m', desc='water depth')
    load_condition =  Str('N',iotype='in',desc='Load condition - N for normal or E for extreme')
    significant_wave_height = Float(10.660,iotype='in', units='m', desc='significant wave height')
    significant_wave_period = Float(13.210,iotype='in', units='m', desc='significant wave period')
    keel_cg_mooring = Float(51.,iotype='in', units='m', desc='center of gravity above keel of mooring line')
    keel_cg_operating_system = Float(20.312,iotype='in', units='m', desc='center of gravity above keel of operating system')
    reference_wind_speed = Float(11.,iotype='in', units='m/s', desc='reference wind speed')
    reference_height = Float(75.,iotype='in', units='m', desc='reference height')
    alpha = Float(0.110,iotype='in', desc='power law exponent')
    ...
    rotor_mass = Float(125000.,iotype='in', units='kg', desc='rotor mass')
    tower_mass = Float(83705.,iotype='in', units='kg', desc='tower mass')
    free_board = Float(13.,iotype='in', units='m', desc='free board length')
    draft = Float(64.,iotype='in', units='m', desc='draft length')
    fixed_ballast_mass = Float(1244227.77,iotype='in', units='kg', desc='fixed ballast mass')
    hull_mass = Float(890985.086,iotype='in', units='kg', desc='hull mass')
    permanent_ballast_mass = Float(838450.256,iotype='in', units='kg', desc='permanent ballast mass')
    variable_ballast_mass = Float(418535.462,iotype='in', units='kg', desc='variable ballast mass')
    number_of_sections = Int(4,iotype='in',desc='number of sections in the spar')
    outer_diameter = Array([5., 6., 6., 9.],iotype='in', units='m',desc = 'outer diameter of each section')
    length = Array([6., 12., 15., 44.],iotype='in', units='m',desc = 'wlength of each section')
    end_elevation = Array([7., -5., -20., -64.],iotype='in', units='m',desc = 'end elevation of each section')
    start_elevation = Array([13., 7., -5., -20.],iotype='in', units='m',desc = 'start elevation of each section')
    ...
    shell_and_ring_mass = Float(iotype='out',desc = 'mass to be minimized')
    def __init__(self):
